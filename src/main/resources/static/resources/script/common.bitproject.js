...
function getCurrentDateTime(format) {
    //get new date object[YYYY-MM-DD]
    var today = new Date();
    var month = today.getMonth()+1 //[0-11]
    if(month<10) month = "0"+month;
    var date = today.getDate(); //[0-31]
    if(date<10) date = "0"+date;

    var hours = today.getHours() //[1-24]
    if(hours<10) hours = "0"+hours;
    var minutes = today.getMinutes(); //[1-60]
    if(minutes<10) minutes = "0"+minutes;


    var currentdate = today.getFullYear()+ "-"+month+"-"+date;
    var currenttime = hours + minutes;
    var currentdatetime = today.getFullYear()+ "-"+month+"-"+date + "T"+hours + ":"+ minutes;
...
//min max date loadform
function maxAndMinDate(format,feildID,nodate) {
    let today = new Date(feildID.value);

    let afteroneweek = new Date();
    afteroneweek.setDate(today.getDate()+parseInt(nodate));
    let month = afteroneweek.getMonth()+1; //[0-11]
    if (month < 10) month = "0" + month;
    let day = afteroneweek.getDate(); //range[1-31]
    if (day < 10) day = "0" + day;
    let maxdate = afteroneweek.getFullYear()+"-"+month+"-"+day;

    let before = new Date();
    before.setDate(today.getDate()-parseInt(nodate));
    let monthmin = before.getMonth()+1; //[0-11]
    if (monthmin < 10) monthmin = "0" + monthmin;
    let daymin = before.getDate(); //range[1-31]
    if (daymin < 10) daymin = "0" + daymin;
    let mindate = before.getFullYear()+"-"+monthmin+"-"+daymin;
...
//date keyup validation for min and max
//dateValidationKU(min OR max, feildID, object name, property name, no of days, old bject name, property name of added date)
function dateValidationKU(format,field,obj,prop,nodate,oldobj,prop2) {
    ...
    var d1 = new Date(field.value);//requiredate(date we type)
    //var d2 = new Date(ob[prop2]);//addeddate
    var d2 = new Date(getCurrentDateTime('date'));//currentdate
    ...
    var requiredate = today.getFullYear()+ "-"+reqmonth+"-"+reqdate;//requiredate(date we type)
    ...
    if (requiredate != ""){
        ...
        if (regpattern.test(requiredate)){
           ...
           if (format =="max") {
                let afteroneweek = new Date();
                afteroneweek.setDate(d2.getDate() + parseInt(nodate));
                let month = afteroneweek.getMonth() + 1; //[0-11]
                if (month < 10) month = "0" + month;
                let day = afteroneweek.getDate(); //range[1-31]
                if (day < 10) day = "0" + day;
                var d3 = new Date(afteroneweek.getFullYear() + "-" + month + "-" + day);//addeddate plus date(maxdate)

               //addeddate <= requiredate && requiredate <= maxdate
           ...
           if (format =="min") {
                ...
                let day = afteroneweek.getDate(); //range[1-31]
                if (day < 10) day = "0" + day;
                var d4 = new Date(afteroneweek.getFullYear() + "-" + month + "-" + day);//addeddate - date(mindate)

                //mindate <= requiredate && requiredate <= addeddate
                if (parseInt(d4.getTime()) <= parseInt(d1.getTime()) && parseInt(d1.getTime()) <= parseInt(d2.getTime())) {
                    console.log("In Range" + d4.getTime() + "<= " + d1.getTime() + " <=" + d2.getTime());
                    if (oldob != null && oldob[prop] != ob[prop]) {
                        field.style.border = updated;
                        console.log("updated");
                        ob[prop] = requiredate;
...
//min and max date upadtefill
function dateUpdateFill(format,field,obj,prop,oldobj,nodate,prop2) {
    ...
    var d4 = new Date(ob[prop2]);//addeddate
    var requiredate = new Date(ob[prop]);//requiredate
    console.log(requiredate);
    var today = new Date(getCurrentDateTime('date'));

    if(format == "max"){
        let afteroneweek = new Date(today);
        afteroneweek.setDate(today.getDate() + parseInt(nodate));
        let month = afteroneweek.getMonth()+1; //[0-11]
        if (month < 10) month = "0" + month;
        let day = afteroneweek.getDate(); //range[1-31]
        ...
        field.max = afteroneweek.getFullYear()+"-"+month+"-"+day;
        //field.min = ob[prop2];//addeddate
        field.min = getCurrentDateTime('date');
        console.log(d4max);
        console.log(today);

        //today <= requiredate
         if(parseInt(today.getTime()) <= parseInt(requiredate.getTime())){
            if(oldob != null && oldob[prop] != ob[prop]){
                field.style.border = updated;
                field.disabled = false;
            }
    ...
    if(format == "min"){
        let beforeweek = new Date(d4);
        beforeweek.setDate(d4.getDate()-parseInt(nodate));
        let aftermonth = beforeweek.getMonth()+1; //[0-11]
        if (aftermonth < 10) aftermonth = "0" + aftermonth;
        let afterday = beforeweek.getDate(); //range[1-31]
        if (afterday < 10) afterday = "0" + afterday;
        var d4min = new Date(beforeweek.getFullYear()+"-"+aftermonth+"-"+afterday);
        //var d4max = new Date(maxAndMinDate('max',dteAddedDate,7));
        field.min = beforeweek.getFullYear()+"-"+aftermonth+"-"+afterday;
        field.max = ob[prop2];//addeddate
        console.log(d4min);
        console.log(today);

        //addeddate-date <= currentdate
        if(parseInt(d4min.getTime()) <= parseInt(today.getTime())){
            if(oldob != null && oldob[prop] != ob[prop]){
                field.style.border = updated;
                field.disabled = false;
            }
...
//fill table function
//filltable(table ID, Data array name, modification button function name,delete button function name,print button function name)
...
function fillDataList(inputcombo, listcombo , list, attribute, selectedvalue) {
    listcombo.innerHTML = "";

    for (index in list) {
        var option = document.createElement("option");
        option.value = list[index][attribute];

        if (list[index][attribute] == selectedvalue) {
            inputcombo.value = selectedvalue;
            inputcombo.style.border = "2px solid green";
        }

        listcombo.appendChild(option);
    }


}

function fillDataList2(combo, list, attribute,attribute2, selectedvalue) {
    combo.innerHTML = "";

    for (index in list) {
        var option = document.createElement("option");
        option.value = list[index][attribute] + " " + list[index][attribute2];
...
function dataListBinder(field,pattern,obj,prop,oldobj,dataList,attribute){
    field.setAttribute('data-pattern',pattern);
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
    var objectDataList = window[dataList];
    ...
    if (val != ""){
        if (regpattern.test(val)) {
            ...
            for (var index in objectDataList){
                if (objectDataList[index][attribute] == val){
                    ob[prop] = objectDataList[index];
                    break;
                }
                else {
                    field.style.border = invalid;
...
function comboBox2NewBinder(field,pattern,obj,prop,oldobj,selectparent){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
    ...
    if (oldob != null && oldob[prop].id != ob[prop].id){
        ...
        // selectparent.children[2].style.border = updated;
    }else {
        $("#"+selectparent).css('border',valid);
        //  selectparent.children[2].style.border = valid;
    }
}
